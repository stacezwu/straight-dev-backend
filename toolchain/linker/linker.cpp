...
std::string getLoadSomeAddressInstr( const std::string& str, uint64_t instr_pc ) {
	...
	if( !is_func && !is_global_var ) {
		if( is_alias ) {
			return getLoadSomeAddressInstr( alias_table[str], instr_pc );
		}
		...
		ostr << "AUiPC    " << std::left << std::setw( 17 ) << (int64_t)upper / (1ll<<12) << '\n';
		ostr << " [......] ";
		ostr << "ADDi.64 <  1>, " << std::left << std::setw( 17 ) << lower;
		return ostr.str();
		...
		std::ostringstream ostr;
		ostr << "LUi      " << std::left << std::setw( 17 ) << (int64_t)upper / (1ll<<12) << '\n';
		ostr << " [......] ";
		ostr << "ADDi.64 <  1>, " << std::left << std::setw( 17 ) << lower;
		return ostr.str();
	} else {
...

	{
		...
		for( const auto& code : codes ) {
			if( code[0] != ' ' ) {
				...
				} else if( matchTarget( code, 19, "Function_" ) ) {
					const auto target = spliceTarget( 19, code );
					std::cout << code.substr(0, 19) << relAddr( target, pc ) / 2 << '\n';
				} else if( matchTarget( code, 33, "BB_" ) ) {
					const auto target = spliceTarget( 33, code );
					std::cout << code.substr(0, 33) << relAddr( target, pc ) / 2 << '\n';
				} else {
					std::cout << code << '\n';
				}
				pc += 4;
